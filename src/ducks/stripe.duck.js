...
export const handleCardSetup = params => dispatch => {
  console.log('handleCardSetup params', params);
  // It's required to use the same instance of Stripe as where the card has been created
  // so that's why Stripe needs to be passed here and we can't create a new instance.
  const { stripe, card, setupIntentClientSecret } = params;

  dispatch(handleCardSetupRequest());

  return stripe
    .handleCardSetup(setupIntentClientSecret, card, {
      payment_method_data: {
    ...
    .catch(err => {
      ...
      // Log error
      const containsSetupIntent = err.error && err.error.payment_intent;
      const { code, doc_url, message, payment_intent } = containsSetupIntent ? err.error : {};
